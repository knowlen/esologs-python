...
from esologs.enums import CharacterRankingMetricType, RoleType, RankingTimeframeType
...
class TestCharacterRankings:
    ...
    @pytest.mark.asyncio
    async def test_get_character_encounter_rankings_basic(self, mock_client, mock_encounter_rankings_response):
    ...
    @pytest.mark.asyncio
    async def test_get_character_encounter_rankings_with_params(self, mock_client, mock_encounter_rankings_response):
        ...
        assert variables["byBracket"] == True
        assert variables["className"] == "Dragonknight"
        assert variables["difficulty"] == 125
        assert variables["includeCombatantInfo"] == True
    ...
    @pytest.mark.asyncio
    async def test_get_character_zone_rankings_basic(self, mock_client, mock_zone_rankings_response):
    ...
    @pytest.mark.asyncio
    async def test_get_character_zone_rankings_latest_zone(self, mock_client, mock_zone_rankings_response):
    ...
    @pytest.mark.asyncio
    async def test_get_character_zone_rankings_with_all_params(self, mock_client, mock_zone_rankings_response):
        ...
        assert variables["byBracket"] == False
        assert variables["className"] == "Nightblade"
        assert variables["difficulty"] == 122
        assert variables["includePrivateLogs"] == True
